...
- `ALAYA_INGEST_RATE_LIMIT_PER_MINUTE` (default `30`) — rate limit for `/ingest/text` per API key per minute. `bench.py` sets this to `10000` in its ephemeral compose stack (via `docker-compose.yml` `ALAYA_INGEST_RATE_LIMIT_PER_MINUTE` passthrough) so large realistic fixtures (396-1936 events) are not rate-limited; `bench_ingest.py` applies an additional client-side throttle: 20 RPS for fixtures >50 events on the bench-managed stack, and 0.4 RPS (24/min, safely under the default limit) in `--reuse` mode against an external stack.
